...
class Command(BaseCommand):
    help = 'Limpia y crea ÚNICAMENTE los planes especificados'
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            # Crear ÚNICAMENTE los tipos de planes necesarios
            self.stdout.write('📋 Creando tipos de planes...')
            plan_types = {
                'consultoria': PlanType.objects.create(name='Consultoría', order=1, is_active=True),
                'personal': PlanType.objects.create(name='Personal', order=2, is_active=True),
                'plan_ejecutivo': PlanType.objects.create(name='Plan Ejecutivo', order=3, is_active=True),
                'enterprise': PlanType.objects.create(name='Enterprise', order=4, is_active=True),
                'enterprise_plus': PlanType.objects.create(name='Enterprise Plus', order=5, is_active=True),
                'plan_corporativo': PlanType.objects.create(name='Plan Corporativo', order=6, is_active=True),
            }
            ...
            try:
                ...
                self.stdout.write('✅ Módulos encontrados')
            except Module.DoesNotExist as e:
                self.stdout.write(self.style.ERROR(f'❌ Error: Módulo no encontrado - {e}'))
                return
            
            # === PLANES PARA HOTELES (3 planes exactos) ===
            self.stdout.write('\n🏨 Creando planes para HOTELES...')
            hoteles_plans = [
                {
            ...
            for plan_data in hoteles_plans:
                ...
                self.stdout.write(f'  ✅ {plan_data["name"]} - ${plan_data["yearly_price"]:,.0f}/año')
            
            # === PLANES PARA CONJUNTOS (4 planes exactos) ===
            self.stdout.write('\n🏢 Creando planes para CONJUNTOS...')
            conjuntos_plans = [
                {
            ...
            # === PLANES PARA SEGURIDAD PROBABILÍSTICA (3 planes exactos) ===
            self.stdout.write('\n🔒 Creando planes para SEGURIDAD PROBABILÍSTICA...')
            seguridad_plans = [
                {
                    'plan_type': plan_types['consultoria'],
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'\n🎉 ¡ESTRUCTURA DE PLANES COMPLETADA!'
                    f'\n📊 Tipos de planes creados: {total_plan_types}'
                    f'\n📋 Total planes creados: {total_plans}'
                    f'\n   • HOTELES: 3 planes'
                    f'\n   • CONJUNTOS: 4 planes'  
                    f'\n   • SEGURIDAD PROBABILÍSTICA: 3 planes'
                    f'\n'
                    f'\n⚠️  IMPORTANTE: No hay suscripciones asignadas.'
                    f'\n   Ejecuta: python manage.py assign_user_plans <email> para asignar suscripciones'
                )
            )